...
def test_run_terminal_strips_kobo_stuff_prefix_from_script_path(monkeypatch) -> None:
    ...
    assert captured[0][1]["cwd"] == str(sandbox.KOBO_STUFF_DIR)


def test_run_terminal_strips_kobo_prefix_from_script_path(monkeypatch) -> None:
    monkeypatch.setattr(sandbox, "AGENT_VENV_DIR", sandbox.REPO_VENV_DIR)
    captured: list[tuple[list[str], dict[str, object]]] = []
    ...
    result = sandbox.run_terminal(
        command="python3 src/kobo/integrations/demo.py",
        working_dir="kobo",
        timeout_seconds=20,
    )
...
def test_run_terminal_rejects_non_allowlisted_command_when_allowlist_configured(monkeypatch) -> None:
    ...
    with pytest.raises(PermissionError, match="KOBO_TERMINAL_COMMAND_ALLOWLIST"):
...
def test_run_terminal_logs_timeout_before_raising(monkeypatch) -> None:
    ...
    assert ("terminal_command_timeout", {"working_dir": "kobo_stuff", "command_bin": "agent-context", "timeout_seconds": 20}) in events


def test_run_terminal_does_not_bootstrap_pip_for_existing_agent_venv(monkeypatch) -> None:
    agent_venv = sandbox.PROJECT_ROOT / ".kobo" / f"test_agent_venv_{uuid.uuid4().hex}"
